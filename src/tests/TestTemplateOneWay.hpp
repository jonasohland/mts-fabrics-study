...
namespace riedel::fabricsperf
{
    ...
    public:
        void setup(TestContext& ctx) final
        {
            ...
            if (ctx.runner())
            {
                ctx.setLocalTargetInfo("127.0.0.1:2222");
            }
            else
            {
                ctx.setLocalTargetInfo("127.0.0.1:4444");
            }
        }
        ...
        using Factory = OneWayTestFactory;
    ...
    private:
        void runAsTestRunner(TestContext& ctx)
        {
            ...
            for (uint64_t i = 0; i < ctx.config().iterations; ++i)
            {
                ...
                // Record the time of the sent grain.
    ...
    class OneWayTestFactory : public TestFactory
    {
    public:
        [[nodiscard]]
        std::string name() const final
        {
            return "OneWayTestTemplate";
        }

        std::unique_ptr<Test> operator()() const final
        {
            return std::make_unique<OneWayTest>();
        }
    };
